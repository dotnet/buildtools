...
set clean_src=false
set clean_tools=false
set clean_all=false

if [%1] == [] (
  set unprocessedBuildArgs=-b
  goto Begin
)
...
if /I [%call_run%] == [true] (
  echo. >> %cleanlog% && echo %~dp0run.cmd clean !unprocessedBuildArgs!>> %cleanlog%
  call %~dp0run.cmd clean !unprocessedBuildArgs!>> %cleanlog%
  call :CheckErrorLevel
)
